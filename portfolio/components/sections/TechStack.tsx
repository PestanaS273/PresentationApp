...
const row2 = [
  ...
  { name: 'Apache Kafka',color: '#000000', bg: '#FFFFFF' },
...
function TechBadge({ name, color }: { name: string; color: string }) {
  return (
    ...
    >
      ...
      />
      <span
        className="text-sm font-medium text-[#EDEDEF] whitespace-nowrap"
...
function MarqueeRow({
  ...
  return (
    <div className="flex overflow-hidden">
      ...
      >
        {doubled.map((item, i) => (
          <TechBadge key={`${item.name}-${i}`} name={item.name} color={item.color} />
        ))}
      </div>
    </div>
  )
...
export default function TechStack() {
  ...
  return (
    <section id="stack" ref={ref} className="section-padding relative overflow-hidden">
      {/* Ambient glow */}
      <div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-40 blur-[100px] opacity-10 pointer-events-none"
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          ...
          <p
            className="text-[#8A8F98] mt-4 max-w-md mx-auto text-base"
            style={{ fontFamily: 'var(--font-inter)' }}
          >
